...
where
    P: Provider + Clone,
{
    async fn send_private_transaction(&self, tx_request: TransactionRequest) -> Result<B256, MarketError> {
        let params = serde_json::json!([{
           "from": format!("{:#x}", tx_request.from.unwrap_or(self.caller)),
           "to": format!("{:#x}", self.instance.address()), // Some() kaldır
           "data": tx_request.input.input.as_ref().map(|data| format!("{:#x}", data)),
           // "gas": tx_request.gas.map(|g| format!("{:#x}", g)),
           "maxFeePerGas": tx_request.max_fee_per_gas.map(|f| format!("{:#x}", f)),
           "maxPriorityFeePerGas": tx_request.max_priority_fee_per_gas.map(|p| format!("{:#x}", p)),
           "value": tx_request.value.map(|v| format!("{:#x}", v)).unwrap_or("0x0".to_string()),
           "nonce": tx_request.nonce.map(|n| format!("{:#x}", n))
        }]);
        tracing::info!("🔍 Private TX params: {}", serde_json::to_string_pretty(&params).unwrap());
        let response: serde_json::Value = self.provider
            .raw_request("eth_sendPrivateTransaction".into(), Some(params))
            .await
            .context("Private tx failed")?;

        let tx_hash_str = response.as_str()
            .context("Invalid response format")?;

        let tx_hash = tx_hash_str.parse::<B256>()
    ...
    P: Provider + Send + Sync + Clone,
{
    ...
    pub async fn lock_request_private(
        &self,
        request: &ProofRequest,
        client_sig: impl Into<Bytes>,
        priority_gas: Option<u64>,
        ...
        let tx_hash_str = response.as_str().context("Invalid response")?;
        let tx_hash = tx_hash_str.parse::<B256>().context("Invalid tx hash")?;
